...
def sanitize_results(results):
    ...
    for i, result in enumerate(results):
        if result is None:
            # 将None转换为空的结果字典
            sanitized.append({'pts_bbox': {'boxes_3d': [], 'scores_3d': [], 'labels_3d': []}})
            none_count += 1
        elif isinstance(result, dict):
            # 确保pts_bbox存在
            if 'pts_bbox' not in result or result['pts_bbox'] is None:
                result['pts_bbox'] = {'boxes_3d': [], 'scores_3d': [], 'labels_3d': []}
            sanitized.append(result)
        else:
            sanitized.append(result)
    
    if none_count > 0:
        logging.warning(f'Found {none_count} None results out of {len(results)}, replaced with empty dicts')
