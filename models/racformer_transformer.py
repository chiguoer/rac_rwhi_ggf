...
from .bbox.utils import decode_bbox, theta_d2xy_coods, xy2theta_d_coods, compute_map_size_and_radius
...
class RaCFormerTransformerDecoder(BaseModule):
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, lss_bev_feats, radar_bev_feats, attn_mask, img_metas):
        ...
        for i in range(self.num_layers):
            ...
            bbox_pred = theta_d2xy_coods(bbox_pred, self.map_size, self.polar_radius)
...
class RaCFormerTransformerDecoderLayer(BaseModule):
    def __init__(self, 
        ...
        self.sampling = RaCFormerSampling(
            embed_dims,
        ...
        self.sampling_radar_bev = BEVSampling(
            ...
            num_frames=num_frames,
            num_heads=4,
            num_points=num_points_bev,
            num_levels=1,
            pc_range=pc_range,
            depth_num=bev_depth_num,
            spatial_shapes=spatial_shapes,
            temp_radar=True,
            map_size=self.map_size,
            polar_radius=self.polar_radius,
...
class ScaleAdaptiveSelfAttention(BaseModule):
    ...
    def __init__(self, embed_dims=256, num_heads=8, dropout=0.1, pc_range=[], map_size=None, polar_radius=None, init_cfg=None):
    ...
    def inner_forward(self, query_bbox, query_feat, pre_attn_mask):
        ...
        query_feat: [B, Q, C]
        """
        query_bbox = theta_d2xy_coods(query_bbox, self.map_size, self.polar_radius).clone()
...
class RaCFormerSampling(BaseModule):
    ...
    def __init__(self, embed_dims=256, num_frames=4, num_groups=4, num_points=8, num_levels=4, depth_num=15, pc_range=[], map_size=None, polar_radius=None, init_cfg=None):
        super().__init__(init_cfg)
    ...
    def inner_forward(self, query_ray, query_feat, mlvl_feats, img_metas, d_region=0.1):
        ...
        query_bbox = theta_d2xy_coods(query_ray, self.map_size, self.polar_radius).clone()
        ...
        sampling_points = xy2theta_d_coods(sampling_points, self.map_size, self.polar_radius)
        ...
        sampling_points = theta_d2xy_coods(sampling_points, self.map_size, self.polar_radius)
