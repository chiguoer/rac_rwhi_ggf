    ...
    def decode_single(self, cls_scores, bbox_preds):
        ...
        Args:
            ...
            bbox_preds (Tensor): Outputs from the regression \
                ...
                Shape [num_query, 9].
        Returns:
            dict: Decoded boxes. 包含 'bboxes', 'scores', 'labels'，
                  即使没有检测到目标也会返回空tensor而不是None
        """
        ...
        if self.post_center_range is not None:
            limit = torch.tensor(self.post_center_range, device=device)
            ...
            # ============================================================
            # 防护代码：确保即使没有检测到目标也返回有效的空tensor
            # 避免评估阶段因为None值而崩溃
            # ============================================================
            if boxes3d.shape[0] == 0:
                # 返回空tensor，维度正确，避免后续处理报错
                boxes3d = torch.zeros((0, final_box_preds.shape[-1]), device=device)
