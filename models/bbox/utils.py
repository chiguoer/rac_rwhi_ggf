...
def compute_map_size_and_radius(pc_range, map_size=None, polar_radius=None):
    if pc_range is None:
        raise ValueError('pc_range is required to compute map_size/polar_radius.')
    ...
    if polar_radius is None:
        ...
        polar_radius = math.sqrt(x_max ** 2 + y_max ** 2)
    return float(map_size), float(polar_radius)
...
def theta_d2xy_coods(theta_d_coords, map_size, r):
    if map_size is None or r is None:
        raise ValueError('map_size and r must be provided for theta_d2xy_coods.')
    center = map_size / 2
...
def xy2theta_d_coods(xy_coords_norm, map_size, r, norm=True):
    if map_size is None or r is None:
        raise ValueError('map_size and r must be provided for xy2theta_d_coods.')
